...
var accounts = new Datastore({ filename: __dirname + '/crontabs/users.db' });
accounts.loadDatabase(function (err) {
    if (err) throw err;
});
accounts.findOneAndUpdate = accounts.update;
accounts.deleteMany = accounts.remove;
accounts.deleteOne = accounts.remove;
accounts.insertOne = accounts.insert;
...
exports.updateAccount = function(newData, callback)
{
	let findOneAndUpdate = function(data){
		...
		accounts.findOneAndUpdate({_id:data.id}, {$set:o}, {returnOriginal : false}, callback);
	}
	if (newData.pass == ''){
...
{
	accounts.deleteOne({_id: id}, callback);
}
